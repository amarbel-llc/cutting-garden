// Package ical is cutting-garden's own iCalendar (RFC 5545) reader/writer
// for the VTODO and VEVENT components the caldav plugin captures. It is a
// re-homing of the hand-rolled parser that previously lived in
// amarbel-llc/bob (packages/caldav/internal/caldav), decoupling
// cutting-garden from bob: the caldav plugin needs the structured native
// identity (UID, component) to key its RFC 0011 receipt entries, and the
// forthcoming CUD tools (FDR 0020) need structured create/update access.
//
// It is deliberately dependency-free (stdlib only) and scoped to the
