...
// init registers the `[caldav]` config table with the SDK's config-section
// registry (RFC 0007 § Plugin-Owned Sections): the framework's loader
// dispatches the table here by name, so cgconfig names no caldav type.
func init() {
	cutting_garden_plugins.MustRegisterConfigSection(schemeCalDAV, decodeConfigSection)
}
