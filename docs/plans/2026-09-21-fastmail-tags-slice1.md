...
- **D4 — Unknown tags are CREATED.** A tag realizing no mailbox is created by `Mailbox/set` in the SAME request as the `Email/set` (creation-id back-reference), so the apply stays atomic. The back-reference rides as a patch-object KEY (`"mailboxIds/#c1": true`) rather than as an id value — a form RFC 8620 §5.3 does not spell out, so it was **verified live against the Fastmail API on 2026-09-22**: one request creating a mailbox as creation id `c1` and patching a message with `{"mailboxIds/#c1": true}` answered `created: {c1: {id: "P15au"}}` and `updated: {<id>: null}`, and the readback showed the message carrying both its original mailbox and `P15au`. Established, not assumed; the two-request (create, then reference) fallback is not needed. Placement: every existing mailbox whose tag shares at least one leading `-`-separated segment with the new tag is a candidate of one of two kinds — (a) its tag is a PROPER PREFIX of the new tag at a `-` boundary, so the new mailbox would be its `-`-continuation child; (b) otherwise the new mailbox would be a BARE sibling under its parent. Both kinds are ranked TOGETHER by the number of shared leading segments: the most shared wins; on a tie (a) beats (b); a remaining tie (only possible between (b)s) goes to the lexically first tag. With no candidate, (c) a bare root mailbox. So `payee-charles_tyrwhitt` → `-charles_tyrwhitt` under `payee` ((a) `payee` 1 segment ties (b) `payee-one_medical` 1 → continuation); `proj-trips-26-09-yoga_retreat` → bare child of `area/-travel`, beside `proj-trips-26-09-kyle_yoga` ((b) 4 segments beats (a) `proj` 1); `proj-24-t-foo` → `-foo` under `zz-archive/proj/-24-t` ((a) `proj-24-t` 3 ties (b) `proj-24-t-10x` 3 → an archived project's family stays together). Why ranked rather than (a)-first: the real account has bare interior mailboxes like `zz-archive/proj` whose one-segment tag `proj` is a proper prefix of EVERY `proj-…` tag, so (a)-first would create every new project tag inside the archive subtree (found by vector 8; fixed 2026-09-23). Every created mailbox is reported in `PatchNode`'s `applied` slice as `mailbox:<full/path>` (the summary surface is #260; this slice does not print it).
...
## Task 5b: Plugin-declared box ids (`NodeIDer`) + the mailbox trailing slash

**Why:** the Task 5 fixture exposed that D6's "object ids are thread ids" could not hold. organize's box id was `node_view.RelativeID(nodeURI, anchor)`, which compares host+PATH only; a thread's id rides in the QUERY (`fastmail://test/Inbox/?thread=T1`), so every Inbox thread collapsed to one id and `debug-organize-fastmail-fixture` rendered four indistinguishable `[Inbox …]` boxes. Separately, `setPathSegments` minted `/Inbox` with no trailing slash — contradicting `url.go`'s own `fastmail://acct/area/finance/` comments — so `commonURIPrefix` anchored at `fastmail://test/` instead of the mailbox.

**Design (chosen 2026-09-23):**
- **SDK:** an OPTIONAL `NodeIDer` capability (`internal/cutting_garden_plugins/listing.go`, facaded in `pkgs/`): `RelativeNodeID(nodeURI, anchor string) (id string, ok bool)`. Pure and deterministic in (nodeURI, anchor) — organize never reverses an id, it re-derives each live node's id at apply time and matches by string. `ok=false`, or not implementing it, keeps the host+path default.
- **node_view:** `RelativeIDFor(lister, uri, anchor)` (NodeIDer first, else `RelativeID`) and `BoxIDs(lister, anchor)`, the bound per-document form.
- **organize + list:** every id derivation — generate's object lines and tag-set map, all four apply paths' live re-derivation (`planMoves`, `planFieldEdits`, `planMemberships`, `planAtomMembershipEdits`), the diff preview, and `list -format espalier` — goes through the resolver bound to the one lister that served the nodes (organize threads a `boxIDer` func; no package state), so generate and apply always agree.
- **Uniqueness guard (NodeIDer plugins only):** generate refuses, as a bad request naming the id and both URIs, a node set in which two DISTINCT node URIs resolve to one box id — but only when the lister implements `NodeIDer`, where the plugin chose its ids and a collision is a plugin bug. Default-RelativeID plugins are exempt (decided 2026-09-23): caldav's server-expanded recurring-event occurrences share their master href, discriminated only by `?recurrence-id=` (`plugins/caldav/expand.go` `occurrenceURI`), so they collapse under the host+path default; that pre-existing ambiguity (duplicate `[event.ics]` boxes) must not become a new hard failure. An issue for caldav occurrence ids is to follow.
- **fastmail:** a thread in the anchor's account whose mailbox path starts (segment-wise) with the anchor mailbox's path gets the bare thread id at equal depth, else `<remaining segments>/<threadId>` (account-root anchor → `Inbox/T1`); a `/` or `%` inside one mailbox name is percent-escaped to keep segments injective. Everything else is `ok=false`. Any id string is a single valid box token — `trellis.QuoteIfNeeded` spells a non-identifier id as a quoted String.
- **Trailing slash:** minted mailbox/thread/email/raw URIs end their path in `/`; `classifyURI` still accepts both spellings.

**Target render** (`just debug-organize-fastmail-fixture`): `_anchor = fastmail://test/Inbox/`, `# _inbox`, `- [T1 _unread payee-one_medical from="billing@example.com"] Your statement is ready` … through T4.
...
**Already verified live, so this gate need not re-exercise it:** D4's creation-id-as-patch-KEY form (`"mailboxIds/#c1": true` in an `Email/set` naming a mailbox created by a `Mailbox/set` earlier in the SAME request). Probed against the real Fastmail API on 2026-09-22 with one request; it answered `created: {c1: {id: "P15au"}}` and `updated: {<id>: null}`, and the readback showed the message carrying both its original mailbox and `P15au`. The probe's label was removed from the message afterward; the empty `zz-probe-creationid` mailbox may still exist pending disposal.
