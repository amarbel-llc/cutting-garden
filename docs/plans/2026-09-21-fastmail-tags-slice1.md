...
- **D4 — Unknown tags are CREATED.** A tag realizing no mailbox is created by `Mailbox/set` in the SAME request as the `Email/set` (creation-id back-reference), so the apply stays atomic. The back-reference rides as a patch-object KEY (`"mailboxIds/#c1": true`) rather than as an id value — a form RFC 8620 §5.3 does not spell out, so it was **verified live against the Fastmail API on 2026-09-22**: one request creating a mailbox as creation id `c1` and patching a message with `{"mailboxIds/#c1": true}` answered `created: {c1: {id: "P15au"}}` and `updated: {<id>: null}`, and the readback showed the message carrying both its original mailbox and `P15au`. Established, not assumed; the two-request (create, then reference) fallback is not needed. Placement: (a) if an existing mailbox's tag is a PROPER PREFIX of the new tag at a `-` boundary, the new mailbox is a `-`-continuation child of the LONGEST such mailbox (`payee-charles_tyrwhitt` → `-charles_tyrwhitt` under `payee`); else (b) if some existing mailbox shares a common prefix with the new tag at a `-` boundary, the new mailbox is a BARE sibling under that mailbox's parent (`proj-trips-26-09-yoga_retreat` → bare child of `area/-travel`, beside `proj-trips-26-09-kyle_yoga`); else (c) a bare root mailbox. Every created mailbox is reported in `PatchNode`'s `applied` slice as `mailbox:<full/path>` (the summary surface is #260; this slice does not print it).
...
- **Uniqueness guard (all plugins):** generate refuses, as a bad request naming the id and both URIs, a node set in which two DISTINCT node URIs resolve to one box id. **Open question (found while landing it):** caldav's server-expanded recurring-event occurrences are distinct nodes sharing their master href, discriminated only by `?recurrence-id=` (`plugins/caldav/expand.go` `occurrenceURI`), so they collide under the host+path default — a calendar with a recurring VEVENT in the expansion window now refuses to generate (before, it rendered duplicate `[event.ics]` boxes that apply could not tell apart). Fix candidates: caldav implements `NodeIDer` for occurrences, or the guard is narrowed; undecided.
